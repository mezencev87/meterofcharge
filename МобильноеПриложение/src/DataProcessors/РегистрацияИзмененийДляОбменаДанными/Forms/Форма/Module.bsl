...
Процедура ПоказатьРезультатВыгрузки(Команда)
	...
	Если Результат.Количество() > 0 Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Результат стандартной выгрузки (РИБ)'") + Символы.ПС + ТекстСериализации(Результат);
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура НастроитьРедактированиеИзмененийСервер(ТекущаяСтрока)
	...
	ИначеЕсли Метаданные.Справочники.Содержит(Мета) 
		...
		Или Метаданные.ПланыВидовХарактеристик.Содержит(Мета) Тогда
		// Ссылочный тип
